  ...
  const char* ip, const char* port)
{
  ...
  return uxr_connectTcpMbed(platform->client_instance, ip, (uint16_t)strtol(port, &str_ptr, 10));
}
    ...
    const uint8_t* buf, size_t len, uint8_t* errcode)
{
  ...
  rv = uxr_writeTcpDataMbed(buf, len);
    ...
    size_t len, int timeout, uint8_t* errcode)
{
  ...
  rv = uxr_readTcpDataMbed(buf, len, timeout);
